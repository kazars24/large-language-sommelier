...
class SemanticSplitter:
    ...
    def split(self, text: str) -> List[str]:
        ...
        embeddings = np.array(self.model.encode(combined_sentences))
    ...
    def _combine_sentences(self, sentences: List[str]) -> List[str]:
        ...
        for i in range(len(sentences)):
            combined = " ".join(
                sentences[max(i - self.buffer_back, 0) : i + 1 + self.buffer_forward]
            )
    ...
    def _calculate_distances(self, embeddings: np.ndarray) -> List[float]:
        ...
        """
        return [
            float(cosine(embeddings[i], embeddings[i + 1]))
